...
from ...Models.GasSupplyPointsModel import GasSupplyPointsModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .GasSupplyPointsAdditionalDialog import GasSupplyPointsAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'GasSupplyPointsMainDialog.ui'))

class GasSupplyPointsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.GasSupplyPoints
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode = DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(GasSupplyPointsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = GasSupplyPointsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11J.getSpecificElements(["11J.0","11J.14", "11J.21"]))
        self.dictionaryComboBoxHelper.setItems(self.cbArrangement, ds.Dictionary11F.getElements())
        self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT24.getElementsExcept(["CT24.1"]))
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = GasSupplyPointsAdditionalDialog(self.model, self.mode, self)
