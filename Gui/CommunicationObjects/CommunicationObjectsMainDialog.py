...
from ...Models.CommunicationObjectsModel import CommunicationObjectsModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .CommunicationObjectsAdditionalDialog import CommunicationObjectsAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'CommunicationObjectsMainDialog.ui'))

class CommunicationObjectsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.CommunicationObjects
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(CommunicationObjectsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = CommunicationObjectsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType,
                                                   ds.Dictionary11L.getElementsExcept(["11L.3", "11L.4", "11L.6"]))
            self.dictionaryComboBoxHelper.setItems(self.cbArrangement, ds.Dictionary11F.getElements())
            self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT28.getSpecificElements(["CT28.1"]))
    ...
    def __checkAddictionalStructuresEnable(self):
        ...
        self.additionalStructuresIsSelected = item and item.name == "11L.0"
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = CommunicationObjectsAdditionalDialog(self.model, self.mode, self)
