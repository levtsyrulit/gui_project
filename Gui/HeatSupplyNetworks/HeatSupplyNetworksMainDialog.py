...
from ...Models.HeatSupplyNetworkModel import HeatSupplyNetworksModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .HeatSupplyNetworksAdditionalDialog import HeatSupplyNetworksAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'HeatSupplyNetworksMainDialog.ui'))

class HeatSupplyNetworksMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.HeatSupplyNetworks
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(HeatSupplyNetworksMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = HeatSupplyNetworksModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11U.getElements())
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = HeatSupplyNetworksAdditionalDialog(self.model, self.mode, self)
