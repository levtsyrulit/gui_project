...
from ...Models.ElectricitySupplyObjectsModel import ElectricitySupplyObjectsModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .ElectricitySupplyObjectsAdditionalDialog import ElectricitySupplyObjectsAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'ElectricitySupplyObjectsMainDialog.ui'))

class ElectricitySupplyObjectsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.ElectricitySupplyObjects
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(ElectricitySupplyObjectsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = ElectricitySupplyObjectsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11P.getElementsExcept(["11P.28"]))
            self.dictionaryComboBoxHelper.setItems(self.cbArrangement, ds.Dictionary11F.getElements())
            self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT27.getSpecificElements(["CT27.8"]))
    ...
    def __checkAddictionalStructuresEnable(self):
        ...
        self.additionalStructuresIsSelected = item and item.name == "11P.0"
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = ElectricitySupplyObjectsAdditionalDialog(self.model, self.mode, self)
