...
from .CommunicationPointsAdditionalDialog import CommunicationPointsAdditionalDialog
...
from ...Models.CommunicationPointsModel import CommunicationPointsModel
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'CommunicationPointsMainDialog.ui'))

class CommunicationPointsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.CommunicationPoints
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(CommunicationPointsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = CommunicationPointsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        self.dictionaryComboBoxHelper.setItems(self.cbObjectType,
                                                ds.Dictionary11L.getSpecificElements(["11L.0","11L.3", "11L.4", "11L.6"]))
        self.dictionaryComboBoxHelper.setItems(self.cbArrangement, ds.Dictionary11F.getElements())
        self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT28.getElementsExcept(["CT28.1"]))
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = CommunicationPointsAdditionalDialog(self.model, self.mode, self)
