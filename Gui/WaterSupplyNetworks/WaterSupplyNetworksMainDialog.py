...
from ...Models.WaterSupplyNetworksModel import WaterSupplyNetworksModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .WaterSupplyNetworksAdditionalDialog import WaterSupplyNetworksAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'WaterSupplyNetworksMainDialog.ui'))

class WaterSupplyNetworksMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = "Сети водоснабжения"
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(WaterSupplyNetworksMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = WaterSupplyNetworksModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11T.getElements())
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = WaterSupplyNetworksAdditionalDialog(self.model, self.mode, self)
