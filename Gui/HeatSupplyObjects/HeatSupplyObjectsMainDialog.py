...
from ...Models.HeatSupplyObjectsModel import HeatSupplyObjectsModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .HeatSupplyObjectsAdditionalDialog import HeatSupplyObjectsAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'HeatSupplyObjectsMainDialog.ui'))

class HeatSupplyObjectsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.HeatSupplyObjects
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(HeatSupplyObjectsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = HeatSupplyObjectsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11N.getElementsExcept(["11N.5"]))
            self.dictionaryComboBoxHelper.setItems(self.cbArrangement, ds.Dictionary11F.getElements())
            self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT25.getSpecificElements(["CT25.1", "CT25.4"]))
            self.__initFields()
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = HeatSupplyObjectsAdditionalDialog(self.model, self.mode, self)
