...
from ...Models.LiquidHydrocarbonsNetworksModel import LiquidHydrocarbonsNetworksModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .LiquidHydrocarbonsNetworksAdditionalDialog import LiquidHydrocarbonsNetworksAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'LiquidHydrocarbonsNetworksMainDialog.ui'))

class LiquidHydrocarbonsNetworksMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.LiquidHydrocarbonsNetworks
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(LiquidHydrocarbonsNetworksMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = LiquidHydrocarbonsNetworksModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11Y.getElements())
    ...
    def __isSpecialPurposePipelineSelected(self):
        item = self.dictionaryComboBoxHelper.getSelectedItem(self.cbObjectType)
        return item and item.name == "11Y.4"
    ...
    def __fillModel(self):
        ...
        if self.__isSpecialPurposePipelineSelected() == False:
            self.model.specialPurposePipeline = None
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = LiquidHydrocarbonsNetworksAdditionalDialog(self.model, self.__isSpecialPurposePipelineSelected(), self.mode, self)
