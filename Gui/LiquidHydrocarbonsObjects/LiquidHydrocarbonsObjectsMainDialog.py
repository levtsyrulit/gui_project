...
from ...Models.LiquidHydrocarbonsObjectsModel import LiquidHydrocarbonsObjectsModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .LiquidHydrocarbonsObjectsAdditionalDialog import LiquidHydrocarbonsObjectsAdditionalDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'LiquidHydrocarbonsObjectsMainDialog.ui'))

class LiquidHydrocarbonsObjectsMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.LiquidHydrocarbonsObjects
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode=DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(LiquidHydrocarbonsObjectsMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = LiquidHydrocarbonsObjectsModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbAdditionalStructures, ds.DictionaryCT26.getSpecificElements(["CT26.1","CT26.7", "CT26.8"]))
            self.__initFields()
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = LiquidHydrocarbonsObjectsAdditionalDialog(self.model, self.mode, self)
