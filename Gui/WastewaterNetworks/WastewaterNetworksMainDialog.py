...
from ...Models.WastewaterNetworkModel import WastewaterNetworkModel
from ..ObjectPassport.ObjectPassportDialog import ObjectPassportDialog
from .WastewaterNetworksAdditionalAttributesDialog import WastewaterNetworksAdditionalAttributesDialog
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'WastewaterNetworksMainDialog.ui'))

class WastewaterNetworksMainDialog(QtWidgets.QDialog, FORM_CLASS):
    LAYERNAME = LayerNames.WastewaterNetworks
    closingDlg = pyqtSignal()

    def __init__(self, iface, mode = DlgMode.VIEW, parent=None, feature=None):
        """Constructor."""
        super(WastewaterNetworksMainDialog, self).__init__(parent)
        ...
        self.mapTool = None
        self.model = WastewaterNetworkModel()
        self.layer = None
        self.points = []
        self.mode = mode
        ...
        if signal == False:
            ...
            self.dictionaryComboBoxHelper.setItems(self.cbObjectType, ds.Dictionary11S.getElements())
    ...
    def __initLayer(self):
        ...
        if layer is None:
            QMessageBox.critical(self, "Критическая ошибка", "Не найден слой ""{0}""".format(self.LAYERNAME), QMessageBox.Ok)
            return False
    ...
    def __fillModel(self):
        ...
        if self.mode == DlgMode.CREATE:
            self.model.creationDate = QDateTime.currentDateTime()
            #self.model.creator = "Создатель"
        self.model.updateDate = QDateTime.currentDateTime()
    ...
    def __openAdditionalAttributesDlg(self):
        dlgAdditionalAttributes = WastewaterNetworksAdditionalAttributesDialog(self.model, self.mode, self)
